 ...
 * # 要求
 * 1. 网站功能
 *     * 用户输入: 至少实现一个功能,允许用户输入并提交数据(如留言、注册、登录、评论等).
 *     * 数据存储: 将用户输入的数据保存到数据库,确保持久化存储.
 *     * 列表展示: 提供列表展示功能(如展示用户列表、商品目录或评论列表).
 *     * 详情查看: 支持点击列表项查看详细信息页面.
 *     * 页面展示: 网站页面需设计合理,确保用户体验良好.
 * 2. 技术栈要求
 *     * 前端技术: HTML、CSS、JavaScript 或前端框架(如 React、Vue、Bootstrap).
 *     * 后端技术: 不限(Node.js、Django、Spring Boot 等均可).
 *     * 数据库: 不限(MySQL、MongoDB等均可).
...
export default defineComponent({
    ...
    methods: {
        ...
        calcuHeader() {
            ...
            if (headerPH)
                headerPH.style.marginTop = `${document.getElementById("header")?.offsetHeight}px`;
        }
    },
    mounted() {
        this.updateSize();
