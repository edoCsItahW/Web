...
`Napi::Value`是JavaScript值的C++表示形式.它是其他JavaScript值(如`Napi::Number`,
`Napi::Boolean`,`Napi::String`和`Napi::Object`)所基于的基类.它表示未知类型的
JavaScript值.它是对Node-API数据类型`napi_value`的精简包装.该类上的方法可用于检查
底层Node-API`napi_value`的JavaScript类型,也可用于转换为C++类型.
